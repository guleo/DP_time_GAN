...
def discriminative_score_metrics (dataX, dataX_hat):
    ...
    def discriminator (X, T):
        ...
        with tf.variable_scope("discriminator", reuse = tf.AUTO_REUSE) as vs:
            ...
            # WARNING:tensorflow:From metrics/discriminative_score_metrics.py:77: all_variables (from tensorflow.python.ops.variables) is deprecated and will be removed after 2017-03-02.
    ...
    for itt in range(iterations):
        ...
        #%% Checkpoints
        if itt % 100 == 0:
           print("[Dis-criminative S-core step: {}] loss - d loss: {}".format(itt, np.round(step_d_loss,4)))
            # loss小于预定值跳出
           if np.round(step_d_loss,4) < 0.08:
               print(f'loss已达标，跳出循环')
               break
        #    loss 不再减小跳出
           if np.round(step_d_loss,4) < min_loss:
               min_loss = np.round(step_d_loss,4)
           else:
               min_loss = np.round(step_d_loss,4)
               loss_count = loss_count + 1
               print(f'loss不再减小第 {loss_count} 次')
               if loss_count == 2:
                print(f'loss不再减小，跳出循环')
                break
        #    if np.round(step_d_loss,4) < 0.08:

...
def predictive_score_metrics (dataX, dataX_hat):
    ...
    for itt in range(iterations):
        ...
        #%% Checkpoints
        if itt % 100 == 0:
           print("[Pre-dictive S-core step: {}] loss - d loss: {}".format(itt, np.sqrt(np.round(step_d_loss,4))))

            # loss小于预定值跳出
           if np.round(step_d_loss,4) < 0.08:
                print(f'd_loss = {np.round(step_d_loss,4)},loss已达标，跳出循环')
                break
            #    loss 不再减小跳出
           if np.round(step_d_loss,4) < min_loss:
                ...
                # print("[Pre-dictive S-core step: {}] loss - d loss: {}".format(itt, np.sqrt(np.round(step_d_loss,4))))
                print(f'loss不再减少,第 {loss_count} 次。')
                if loss_count == 2:
                    print(f'loss不再减少,退出循环')
                    break
